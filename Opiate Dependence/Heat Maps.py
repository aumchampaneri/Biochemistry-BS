...
def differential_expression_heatmap(
    ...
    expression_data = expression_data.join(adata.obs[[groupby, "disease"]])
    ...
    if group1 not in expression_data["disease"].values or group2 not in expression_data["disease"].values:
    ...
    group1_mean = expression_data.loc[expression_data["disease"] == group1].groupby(groupby)[
        valid_ensembl_ids].mean()
    group2_mean = expression_data.loc[expression_data["disease"] == group2].groupby(groupby)[
        valid_ensembl_ids].mean()
...
differential_expression_heatmap(
    adata, gene_dict, groupby=f"{subset}", group1="normal", group2=f"{disease}",
    ...
    figsize=(12, 17), dendrogram_ratio=(0.05, 0.2), colormap="RdYlBu", show_dendrogram=False  # Use a diverging colormap
)